...
Ext.define('gw.app', function () {
     ...
     * @private
     */
    function _initializeDefaultCommunicationMessages() {
        _config = _config || {};
        _config.displayKeys = _config.displayKeys || {};
        if (!_config.displayKeys["ExtJS.Alert.HTTPRequestFailed"]) {
            _config.displayKeys["ExtJS.Alert.HTTPRequestFailed"] = 'HTTP request failed: {0}';
        }
        if (!_config.displayKeys["ExtJS.Alert.InvalidServerResponse"]) {
            _config.displayKeys["ExtJS.Alert.InvalidServerResponse"] = 'Invalid server response. Please check server log.\n{0}';
        }
        if (!_config.displayKeys["ExtJS.Alert.HTTPRequestTimedOut"]) {
            _config.displayKeys["ExtJS.Alert.HTTPRequestTimedOut"] =
                'Your server request has exceeded the timeout threshold. Refresh the page to try again. ' +
                'If you continue to receive this error, contact your system administrator.';
        }
    }
...
//          {key:Ext.event.Event.ENTER, shift:false, alt:false, fn:_handleEnterKey} // ENTER key
    ...
    return {
        ...
        initPage: function (view, config, commands) {
            ...
            _config = config;
            _initializeDefaultCommunicationMessages();
