...
Ext.define('gw.CmdProc', {
    ...
    replace : function (cmdInfo) {
        var comp = Ext.ComponentManager.get(cmdInfo.target);
        if (comp == null) {
            gw.app.showAlert("Attempting to replace for a non existing widget with id: " + cmdInfo.target +
        ...
        if (!cmdInfo.items) {
            gw.app.showAlert("There are no items to replace");
        } else if (cmdInfo.items.length != 1) {
            gw.app.showAlert("We only expect one item in replace but got " + cmdInfo.items.length);
        } else {
    ...
    updateValues : function(cmdInfo, response) {
        ...
        for (var i = 0; i < cmdInfo.items.length;i++) {
            ...
            if (comp == null) {
                gw.app.showAlert("Attempting to update values for a non existing widget with id: " + item.id +
                    ". This is likely a PCF Configuration error. Please verify PostOnChange has the correct layout target configured.");
                continue;
            }
